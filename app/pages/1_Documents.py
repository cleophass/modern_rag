...
if 'doc_manager' not in st.session_state:
    try:
        ...
        st.error(
            f"Erreur lors de l'initialisation du gestionnaire de documents: {str(e)}")
...
try:
    index_exists = st.session_state.doc_manager.es_client.verify_index(
        index_name)

    if index_exists:
        st.success("✅ Index connecté")
        try:
            documents = st.session_state.doc_manager.es_client.list_documents(
                index_name)
            st.metric("Nombre de documents", len(documents))
        except Exception as e:
            st.error(f"Erreur lors du chargement des documents: {str(e)}")
            activity_logger.log_interaction(
                f"Document list loading error: {str(e)}", "error")
    else:
...
with tab1:
    ...
    if index_exists:
        try:
            ...
            activity_logger.log_interaction(
                f"Documents list error: {str(e)}", "error")

        st.subheader("Détails et Actions")

        selected_idx = st.selectbox(
            "Sélectionner un document pour voir les détails", options=range(
                len(documents)), format_func=lambda x: documents[x]["_source"].get(
                "doc_title", f"Document {x}"))
            ...
            with col1:
                ...
                with st.expander("ℹMétadonnées", expanded=True):
                    ...
                    with col_a:
                        ...
                        st.write(
                            f"**Modifié:** {metadata.get('modified', 'N/A')}")
                    with col_b:
                        st.write(
                            f"**Modèle d'embedding:** {metadata.get('embedding_model', 'N/A')}")
                        st.write(
                            f"**Dimension:** {metadata.get('embedding_dimension', 'N/A')}")
                        st.write(
                            f"**Date embedding:** {metadata.get('embedding_date', 'N/A')}")
                ...
                if st.button("Supprimer ce document", type="secondary"):
                    try:
                        ...
                        res = st.session_state.doc_manager.delete_document(
                            index_name, selected_doc["_id"], selected_doc["_source"].get(
                                "doc_title", ""), selected_doc["_source"].get(
                                "metadata", {}).get(
                                "source", ""))
                        if res:
