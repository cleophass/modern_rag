...
class History:
    ...
    def list_history(self):
        try:
            ...
            self.activity_logger.log_interaction(
                f"Error listing history: {e}", "error")
            return []
    ...
    def load_messages(self, conversation_id: str) -> List[dict]:
        try:
            ...
            self.activity_logger.log_interaction(
                f"Error loading messages: {e}", "error")
            return []
    ...
    def create_conversation(self, conversation_id: str, message: str) -> bool:
        try:
            try:
                ...
                self.activity_logger.log_interaction(
                    f"Error generating title: {e}", "error")
            ...
            self.es_client.es.index(
                index=self.history_index_name,
                document=conversation_doc)
            print("HISTORY: Conversation created successfully")
            return True
        except Exception as e:
            self.activity_logger.log_interaction(
                f"Error creating conversation: {e}", "error")
            return False
    ...
    def add_message(
        ...
        try:
            ...
            return self.add_message_to_history(
                self.message_index_name, message=message_doc)
        except Exception as e:
            self.activity_logger.log_interaction(
                f"Error adding message: {e}", "error")
            return False

    def create_history_index(self):
        self.es_client.create_index(
            self.history_index_name,
            mappings=self.history_index_mapping)

    def create_message_index(self):
        self.es_client.create_index(
            self.message_index_name,
            mappings=self.message_index_mapping)
    ...
    def add_message_to_history(self, index_name: str, message: Dict, ) -> bool:
        try:
            ...
            self.activity_logger.log_interaction(
                f"Error adding message to history: {e}", "error")
            return False
