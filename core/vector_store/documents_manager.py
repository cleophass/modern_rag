...
class DocumentsManager:
    def __init__(self, raw_path: str, clean_path: str):
        ...
        self.preprocessor = Preprocessor(
            raw_path=raw_path, clean_path=clean_path)
    ...
    def create_document_index(self):
        self.es_client.create_index(
            ...
            mappings=self.document_index_mapping)
    ...
    def add_document(self, index_name: str, document_path: str) -> bool:
        ...
        try:
            ...
            self.activity_logger.log_interaction(
                f"Error preprocessing document: {e}", "error")
            return False
        ...
        try:
            ...
            self.activity_logger.log_interaction(
                f"Error embedding document: {e}", "error")
            return False
    ...
    def embed_document(self, document_path: str) -> Embeddings:
        ...
        try:
            ...
            self.activity_logger.log_interaction(
                f"Error embedding document content: {e}", "error")
            raise e
    ...
    def delete_document(
        ...
        try:
            ...
            raw_file_path = os.path.join(
                self.preprocessor.raw_path,
                doc_name + file_extension)
            clean_file_path = os.path.join(
                self.preprocessor.clean_path, doc_name + ".json")
            # delete files
            try:
                ...
                self.activity_logger.log_interaction(
                    f"Error deleting files: {e}", "error")
            ...
            self.activity_logger.log_interaction(
                f"Error deleting document: {e}", "error")
            return False
