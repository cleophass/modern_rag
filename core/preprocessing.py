...
class Preprocessor:
        ...
        try:  # Detect file type
            ...
            if json_result:
                ...
                clean_file_path = os.path.join(
                    self.clean_path, f"{doc_title}.json")
            ...
            self.activity_logger.log_interaction(
                f"Error processing file {file_path}: {e}", "error")
            raise e
    ...
    def _process_csv(self, file_path: str) -> Dict:
        try:
            ...
            # here I will add a step to truncate the content if too long but in
            # the funture we could chunk documents
            if len(df) > 20:
                df = df.head(20)
    ...
    def _extract_date(self, text: str) -> str:
        try:
            # check if in the text there is a 4 digits starting by 20 and
            ...
            self.activity_logger.log_interaction(
                f"Error extracting date from text: {e}", "error")
            return ""
