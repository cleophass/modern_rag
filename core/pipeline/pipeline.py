...
class RAGPipeline:
    ...
    def process_query(self,
                      ...
                      status_callback: Optional[Callable[[str],
                                                         None]] = None) -> RAGResponse:
        # this will be used in front to display the state
        ...
        try:
            ...
            if guard_response.isSafe:
                ...
                update_status(
                    f"Question failed guardrails: {guard_response.reasons}")
            ...
            self.activity_logger.log_interaction(
                f"Error during guardrail validation: {e}", "error")
            return self.utils.construct_RAGResponse(
                answer="",
        ...
        try:
            ...
            self.activity_logger.log_interaction(
                f"Error during question rewriting: {e}", "error")
            return self.utils.construct_RAGResponse(
                answer="",
        ...
        try:
            ...
            hyde_response = self.hyde_agent.generate_hyde(
                rewritten_question.rewritten_question)

        except Exception as e:
            self.activity_logger.log_interaction(
                f"Error during HyDE generation: {e}", "error")
            return self.utils.construct_RAGResponse(
                answer="",
        ...
        try:
            ...
            rewritten_docs = self.retriever.retrieve_documents(
                rewritten_question.rewritten_question, top_k=4)
            update_status(
                f"Retrieved {len(rewritten_docs.hits)} documents for rewritten question.")

        except Exception as e:
            self.activity_logger.log_interaction(
                f"Error during document retrieval: {e}", "error")
        ...
        try:
            ...
            hyde_docs = self.retriever.retrieve_documents(
                hyde_response.hypothetical_answer, top_k=4)
            update_status(
                f"Retrieved {len(hyde_docs.hits)} documents for HyDE question.")
        except Exception as e:
            self.activity_logger.log_interaction(
        ...
        merged_docs = self.utils.merge_two_ElasticsearchAnswer_lists(
            rewritten_docs, hyde_docs)

        try:
            update_status("Reranking documents...")

            reranked_docs = self.reranker.rerank(
                question, merged_docs, top_n=2)
            # extract all content as a single list  of string
            reranked_contents = [
                doc.source.get(
                    'content',
                    '') for doc in reranked_docs.hits]
            update_status(f"Reranked to {len(reranked_docs.hits)} documents.")
        except Exception as e:
            self.activity_logger.log_interaction(
                f"Error during document reranking: {e}", "error")
            return self.utils.construct_RAGResponse(
                answer="",
        ...
        try:
            ...
            self.activity_logger.log_interaction(
                f"Error during answer generation: {e}", "error")
            return self.utils.construct_RAGResponse(
                answer="",
