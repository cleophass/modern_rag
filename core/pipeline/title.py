...
class TitleAgent:
    ...
    def _initialize_agent(self):
        try:
            ...
            self.activity_logger.log_interaction(
                f"Error initializing Title Agent: {e}", "error")
            raise e
    ...
    def create_title(self, question: str) -> TitleAgentResponse:
        try:
            ...
            if isinstance(
                    response,
                    dict) and 'structured_response' in response:
                response = response['structured_response']
                return response
            else:
                self.activity_logger.log_interaction(
                    f"Invalid response structure from title agent: {response}", "error")
                return TitleAgentResponse(
                    **{"Titre": "Invalid response structure"})
        except Exception as e:
            self.activity_logger.log_interaction(
                f"Error creating title: {e}", "error")
            raise e
