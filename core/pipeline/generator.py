    ...
    def _initialize_agent(self):
        try:
            ...
            self.activity_logger.log_interaction(
                f"Error initializing QA Agent: {e}", "error")
            raise e
    ...
    def answer(self, question: str, chunks: List[str]):
        try:
            ...
            response = self.agent.invoke(
                prompt, config=self.config)  # type: ignore
            answer = self.get_answer(response)
            self.activity_logger.log_interaction(
                f"Generated answer: {answer}", "info")
            return answer
        except Exception as e:
            self.activity_logger.log_interaction(
                f"Error generating answer: {e}", "error")
            return "There was an error generating the answer."

    def get_answer(self, response) -> str:
        try:
            # Let's browse the response in inverse order and find the first
            # AIMessage
